...
class NeuralPrometheusMetrics:
    ...
    def export_prometheus_format(self) -> str:
        ...
        lines.append(f"# HELP neural_requests_total Total Neural requests")
        lines.append(f"# TYPE neural_requests_total counter")
        lines.append(f"neural_requests_total {self.requests_total} {timestamp}")
        
        lines.append(f"# HELP neural_success_total Total Neural successes")
        lines.append(f"# TYPE neural_success_total counter")
        lines.append(f"neural_success_total {self.success_total} {timestamp}")
        
        lines.append(f"# HELP neural_failures_total Total Neural failures")
        lines.append(f"# TYPE neural_failures_total counter")
        lines.append(f"neural_failures_total {self.failures_total} {timestamp}")
        
        lines.append(f"# HELP neural_fallback_total Total Neural fallbacks")
        lines.append(f"# TYPE neural_fallback_total counter")
        ...
        if self.latencies:
            ...
            lines.append(f"# HELP neural_latency_p50 Latency p50")
            lines.append(f"neural_latency_p50 {sorted_lat[p50_idx]:.6f} {timestamp}")
            lines.append(f"# HELP neural_latency_p95 Latency p95")
            lines.append(f"neural_latency_p95 {sorted_lat[p95_idx]:.6f} {timestamp}")
            lines.append(f"# HELP neural_latency_p99 Latency p99")
            lines.append(f"neural_latency_p99 {sorted_lat[p99_idx]:.6f} {timestamp}")
        
        # Métriques globales
        lines.append(f"# HELP neural_global_success_rate Global success rate")
        lines.append(f"neural_global_success_rate {self.global_success_rate:.4f} {timestamp}")
        
        lines.append(f"# HELP neural_active_correlations Active correlations")
